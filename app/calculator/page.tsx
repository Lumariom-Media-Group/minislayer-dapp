...
export default function Calculator() {
  ...
  const totalAmount =(activeTab == "MINT" ? (mintAmount * getMintAndRedeemPrice(currentLevel).mintPrice) : mintAmount * getMintAndRedeemPrice(currentLevel).redeemPrice)  ;

  function getAmountWithTax(amount: number, activeTab: "MINT" | "REDEEM"): number {
    if (activeTab === "REDEEM") {
          ...
          <div className="flex flex-col gap-4 sm:grid sm:grid-cols-2 sm:gap-8">
            ...
            <div className="flex flex-col gap-2 sm:gap-2.5">
              ...
              <input
                ...
                placeholder="-"
                disabled
                value={currentLevel > 0 ? (activeTab == "MINT" ? getMintAndRedeemPrice(currentLevel).mintPrice : getMintAndRedeemPrice(currentLevel).redeemPrice) :"-"}
                className="bg-background outline-none placeholder:text-gray-500 text-xl sm:text-2xl font-medium px-4 py-3 md:px-5 md:py-4 rounded-lg shadow-insetDarkGlow border-2 border-transparent transition-all duration-300 focus:border-primary"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:gap-2.5">
            ...
            <div className="bg-background flex flex-col gap-1 font-medium px-4 py-3 md:px-5 md:py-4 rounded-lg shadow-insetDarkGlow">
              <div className="flex items-center gap-1 font-bold text-base sm:text-lg">
                ...
                <span className={"text-red"}>{activeTab == "MINT" ? "+20% Tax" : "-5% Tax"}</span>
              </div>
              <div className="font-semibold text-blue text-3xl sm:text-4xl">
                {getAmountWithTax(totalAmount,activeTab)} USDV
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:gap-2.5">
            ...
            <div className="bg-background flex flex-col gap-1 font-medium px-4 py-3 md:px-5 md:py-4 rounded-lg shadow-insetDarkGlow">
              <div className="font-semibold text-base sm:text-lg">
                <span>{getAmountWithTax(totalAmount,activeTab)} USDV x 1.25</span>
              </div>
              <div className="font-bold text-green text-3xl sm:text-4xl">
                $ {getAmountWithTax(totalAmount,activeTab) * 1.25}
              </div>
            </div>
          </div>
        </div>
